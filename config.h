...
static int diskcachebytes         = 40 * 1024 * 1024;
...
#define DOWNLOAD(d, r) { \
	.v = (char *[]){ "/bin/sh", "-c", \
	     "xterm -e /bin/sh -c \"curl -L -J -O --user-agent '$1'" \
	     " --referer '$2' -b $3 -c $3 '$0';" \
	     " sleep 5;\"", \
...
#define BMS_PICK { .v = (char *[]){ "/bin/sh", "-c", \
				   "xprop -id $0 -f _SURF_GO 8s -set _SURF_GO \
`cat ~/.surf/sbookmarks | dmenu || exit 0`", \
				   winid, NULL } }

#define BMS_ADD { .v = (char *[]){ "/bin/sh", "-c", \
				  "(echo `xprop -id $0 _SURF_URI | cut -d '\"' -f 2` && \
cat ~/.surf/sbookmarks) | sort -u > ~/.surf/sbookmarks_new && \
mv ~/.surf/sbookmarks_new ~/.surf/sbookmarks", \
				  winid, NULL } }

#define MYPLAY { .v = (char *[]){ "/bin/sh", "-c", \
				  "dtach -n /tmp/plleei play `xprop -id $0 _SURF_URI | cut -d '\"' -f 2`", \
				  winid, NULL } }
#define DCAST { .v = (char *[]){ "/bin/sh", "-c", \
				  "dtach -n /tmp/dcast-`date  +'%Y%m%d%M%s'` ssh -i ~/.ssh/dump_id 10.0.0.10 \'youtube-dl -o - \"`xprop -id $0 _SURF_URI | cut -d '\"' -f 2`\"| castnow --quiet --address 10.0.0.84 -\'", \
				  winid, NULL } }
...
static Key keys[] = {
	...
	{ MODKEY,               GDK_s,      spawn,      BMS_PICK },
	{ MODKEY|GDK_SHIFT_MASK,GDK_s,      spawn,      BMS_ADD },
	{ MODKEY,               GDK_w,      spawn,      MYPLAY },
	{ MODKEY,               GDK_c,      spawn,      DCAST },
